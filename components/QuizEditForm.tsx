...
export default function QuizEditForm({
  ...
  async function handleSave(saveStatus: 'draft' | 'published' | 'archived') {
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      if (q.options.some(o => !o.trim())) {
        setError(`Question ${i + 1} has an empty option.`); return;
      }
    }
    ...
    try {
      ...
      // 3. save questions — new ones (no id) via POST, existing via PATCH
      await Promise.all(
        questions.map(q => {
          if (!q.id) {
            return fetch('/api/questions', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ quizId: quiz.id, ...q }),
            });
          } else {
            return fetch(`/api/questions/${q.id}`, {
              method: 'PATCH',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                text: q.text,
      ...
      setTimeout(() => router.push('/quizzes'), 1200);
    } catch (e: any) {
    ...
    try {
      ...
      await Promise.all(
        questions.filter(q => q.id).map(q =>
          fetch(`/api/questions/${q.id}`, { method: 'DELETE' })
        )
      );
  ...
  async function handleGenerate() {
    ...
    try {
      const res  = await fetch('/api/questions/generate', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({ quizId: quiz.id, count: aiCount, difficulty: aiDifficulty }),
      });
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div className="card" style={{ padding: '24px' }}>
        <div style={{
          ...
          <span style={{
            ...
            background: status === 'published' ? 'var(--success-bg)'
              : status === 'archived' ? 'var(--border)'
              : 'var(--warning-bg)',
      ...
      {/* ── Question cards ── */}
      {questions.map((q, qi) => (
        <div key={qi} className="card" style={{ padding: '20px' }}>
                    ...
                    >
                      {isCorrect && (
                        <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
                          <path d="M2 5l2 2L8 2.5" stroke="white"
      ...
      {success && (
        <div style={{
          ...
          border: '0.5px solid var(--success)', borderRadius: 'var(--radius)',
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          ...
          <button onClick={() => handleSave('published')} className="btn btn-primary"
            disabled={saving} style={{ minWidth: '140px' }}>
                            ...
                            {q.options.map((opt, oi) => {
                              ...
                              return (
                                <div key={oi} style={{
                                  ...
                                  <span style={{
                                    fontSize: '12px', color: isCorrect ? 'var(--success)' : 'var(--text2)',
